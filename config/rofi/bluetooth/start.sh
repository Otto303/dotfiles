...
do
  ...
  if [[ "$device_selected" =~ " Refresh" ]]; then
    refresh=true
    notify-send "Bluetooth Connection" "Refreshing..."
    bluetoothctl -t 3 scan on
  elif [[ "$device_selected" =~ "󰂯 Enable Bluetooth" ]]; then
    bluetoothctl power on
    notify-send "Bluetooth Connection" "Powered on"
    sleep 3
    refresh=true
  else
    refresh=false
...
if [[ "$device_selected" =~ "󰂲 Disable Bluetooth" ]]; then
  ...
  notify-send "Bluetooth Connection" "Powered off"
  ...
  if [[ "$device_action" =~ "Pair" ]]; then
    ...
    if bluetoothctl pair "$device_mac"; then
      ...
      notify-send "Bluetooth Connection" "Pairing failed with $device_selected"
    fi
    ...
    bluetoothctl disconnect "$device_mac" && notify-send "Bluetooth Connection" "Disconnected from ${device_selected:3}"
  elif [[ "$device_action" =~ "Enable auto-connect" ]]; then
    bluetoothctl trust "$device_mac" && notify-send "Bluetooth Connection" "Auto-connection enabled"
